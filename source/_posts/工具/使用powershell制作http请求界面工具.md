---
title: 使用powershell制作http请求界面工具
date: 2020/11/21 17:26:33
tags: 
    - powershell
    - http请求
categories: 工具
description: 直接使用powershell程序编写应用窗口，完成http请求，优点是不需要安装任何程序，因为windows10自带powershell。如果需要使用文件上传则需要安装powershell7。
---

### 说明

1. 把下面的代码放到后缀名为`.ps1`的文件中
2. 右键打开，打开方式为`Windows PowerShell`
3. 打开之后界面如下图：<img data-src="/images/使用powershell制作http请求界面工具/1.png" class="lozad" style="margin: 0 auto;"/>
4. 参数格式说明：
    * header: 请求头，以换行符分割每一条
    * cookie: 以`;`分割，如`a=aaa;b=bbb`
    * content: 请求内容，如果contentType选的为json，则为`json`格式，如`{"a":"aaa","b":"bbb"}`, 如果contentType选的为`x-www-form-urlencoded`,则为form结构，如`a=aaa&b=bbb`
5. 请求示例如下图：<img data-src="/images/使用powershell制作http请求界面工具/2.png" class="lozad" style="margin: 0 auto;"/>
6. 增加文件上传，上传文件需要powershell7，使用form-data类型，content有则格式为json，并且上传文件名不能含中文
